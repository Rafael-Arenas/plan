...
class ClientExceptionHandler:
    ...
    def set_operation_context(self, operation: str) -> None:
        ...
        self._logger = logger.bind(
            component="ClientExceptionHandler", operation=operation
        )
    ...
    async def handle_sqlalchemy_error(
        ...
        self._logger.error(
            f"Error SQLAlchemy en {operation}: {error}", **context
        )
        ...
        if isinstance(error, IntegrityError):
            ...
            await self._handle_invalid_request_error(
                error, operation, entity_id
            )
        else:
    ...
    async def _handle_data_error(
        self, error: DataError, operation: str, entity_id: str | None = None
    ) -> None:
        ...
        raise create_client_validation_error(
            field="unknown",
            value="unknown",
            reason=f"Error de validación de datos: {error!s}",
            operation=operation,
        )
    ...
    async def _handle_operational_error(
        ...
        raise RepositoryError(
            message=f"Error operacional en base de datos: {error!s}",
            operation=operation,
            entity_type="Client",
            entity_id=entity_id,
    ...
    async def _handle_invalid_request_error(
        ...
        raise RepositoryError(
            message=f"Solicitud inválida: {error!s}",
            operation=operation,
            entity_type="Client",
            entity_id=entity_id,
            original_error=error,
    ...
    async def handle_validation_error(
        ...
        raise create_client_validation_error(
            field=field, value=value, reason=reason, operation=operation
        )
    ...
    async def handle_not_found_error(
        ...
        self._logger.info(f"Cliente no encontrado en {operation}", **context)

        if client_id:
            raise create_client_not_found_error(
                client_id=client_id, operation=operation
            )
        else:
            raise ClientNotFoundError(
                message=(
                    "Cliente no encontrado con los criterios especificados"
                ),
                operation=operation,
                search_criteria=search_criteria,
            )
    ...
    async def handle_unexpected_error(
        ...
        self._logger.critical(
            f"Error inesperado en {operation}: {error}", **context
        )

        raise RepositoryError(
            message=f"Error inesperado en {operation}: {error!s}",
            operation=operation,
            entity_type="Client",
            entity_id=entity_id,
            original_error=error,
    ...
    def log_operation_success(
        ...
        """
        context = {"operation": operation, "status": "success"}
        ...
        self._logger.info(
            f"Operación {operation} completada exitosamente", **context
        )
    ...
    def get_error_summary(self, error: Exception) -> dict[str, Any]:
        ...
        return {
            "error_type": type(error).__name__,
            "error_message": str(error),
            "is_client_error": isinstance(
                error,
                ClientNotFoundError | ClientValidationError | ClientDuplicateError,
            ),
