...
from .interfaces.i_client_statistics import IClientStatistics


class ClientStatistics(IClientStatistics):
    """
    ...
    async def get_client_stats(self, client_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            active_projects_result = await self.session.execute(
            ...
            completed_projects_result = await self.session.execute(
            ...
            self._logger.debug(
                f"Estadísticas calculadas para cliente {client_id}: "
                f"{stats}"
            )
            return stats
            ...
            self._logger.error(
                f"Error de base de datos calculando estadísticas del "
                f"cliente {client_id}: {e}"
            )
            ...
            self._logger.error(
                f"Error inesperado calculando estadísticas del "
                f"cliente {client_id}: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="client_stats",
    ...
    async def get_client_counts_by_status(self) -> dict[str, int]:
        ...
        try:
            ...
            inactive_query = select(func.count(Client.id)).where(
            ...
            self._logger.error(
                f"Error de base de datos obteniendo conteos por "
                f"estado: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo conteos por "
                f"estado: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="client_counts_by_status",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_client_creation_trends(
        ...
        try:
            ...
            query = (
                select(
                    func.date_trunc(date_trunc, Client.created_at).label(
                        "period"
                    ),
                    func.count(Client.id).label("count"),
                )
                .where(Client.created_at >= start_date)
                .group_by(func.date_trunc(date_trunc, Client.created_at))
                .order_by(func.date_trunc(date_trunc, Client.created_at))
            )
            ...
            self._logger.debug(
                f"Tendencias de creación obtenidas: {len(trends)} períodos"
            )
            return trends

        except SQLAlchemyError as e:
            self._logger.error(
                f"Error de base de datos obteniendo tendencias de creación: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo tendencias de creación: {e}"
            )
            raise create_client_statistics_error(
    ...
    async def get_client_relationship_duration_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            self._logger.error(
                f"Error de base de datos calculando estadísticas de duración: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado calculando estadísticas de duración: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="relationship_duration_stats",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_clients_by_relationship_duration(
        ...
        try:
            ...
            filtered_clients.sort(
                key=lambda x: x["relationship_years"], reverse=True
            )
            ...
            self._logger.error(
                f"Error de base de datos obteniendo clientes por duración de relación: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes por duración de relación: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="clients_by_relationship_duration",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_clients_by_project_count(
        ...
        try:
            ...
            clients = {
                client.id: client for client in clients_result.scalars().all()
            }
            ...
            self._logger.debug(
                f"Obtenidos {len(clients_stats)} clientes por conteo de proyectos"
            )
            return clients_stats

        except SQLAlchemyError as e:
            self._logger.error(
                f"Error de base de datos obteniendo clientes por conteo de proyectos: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes por conteo de proyectos: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="clients_by_project_count",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_contact_info_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            with_contact_info = sum(
                1 for client in active_clients if client.has_contact_info
            )
            ...
            with_contact_person = sum(
                1 for client in active_clients if client.contact_person
            )
            ...
            # Porcentajes
            contact_info_percentage = (
                (with_contact_info / total_active * 100)
                if total_active > 0
                else 0
            )
            email_percentage = (
                (with_email / total_active * 100) if total_active > 0 else 0
            )
            phone_percentage = (
                (with_phone / total_active * 100) if total_active > 0 else 0
            )
            contact_person_percentage = (
                (with_contact_person / total_active * 100)
                if total_active > 0
                else 0
            )
            complete_info_percentage = (
                (complete_contact_info / total_active * 100)
                if total_active > 0
                else 0
            )

            self._logger.debug(
                f"Estadísticas de contacto calculadas para {total_active} clientes activos"
            )
            ...
            return {
                ...
                "contact_percentages": {
                    ...
                    "contact_person_percentage": round(
                        contact_person_percentage, 2
                    ),
                    "complete_info_percentage": round(
                        complete_info_percentage, 2
                    ),
                },
            ...
            self._logger.error(
                f"Error obteniendo estadísticas de información de contacto: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo estadísticas de contacto: {e}"
            )
            raise create_client_statistics_error(
    ...
    async def get_client_activity_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            clients_this_week_result = await self.session.execute(
                clients_this_week_query
            )
            ...
            clients_this_month_result = await self.session.execute(
                clients_this_month_query
            )
            clients_this_month = clients_this_month_result.scalar() or 0

            # Clientes con proyectos activos
            clients_with_active_projects_query = (
                select(func.count(func.distinct(Client.id)))
                .select_from(Client)
                .join(Project, Client.id == Project.client_id)
                .where(Project.status.in_(["planned", "in_progress"]))
            )
            clients_with_active_projects_result = await self.session.execute(
                clients_with_active_projects_query
            )
            clients_with_active_projects = (
                clients_with_active_projects_result.scalar() or 0
            )

            # Total de clientes
            total_clients_query = select(func.count(Client.id))
            total_clients_result = await self.session.execute(
                total_clients_query
            )
            ...
            active_clients_result = await self.session.execute(
            ...
            self._logger.error(
                f"Error de base de datos generando resumen de actividad: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado generando resumen de actividad: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="client_activity_summary",
                parameters={},
                reason=str(e),
            )
        ...
        try:
            ...
            query = (
                select(
                    ...
                    func.count(Project.id).label("project_count"),
                    func.coalesce(func.sum(Project.estimated_hours), 0).label(
            ...
            avg_projects = (
                sum(project_counts) / len(project_counts)
                if project_counts
                else 0
            )
            ...
            for row in clients_data:
                client_age_days = (
                    (current_time.date() - row.created_at.date()).days
                    if row.created_at
                    else 0
                )
                client_age_months = (
                    client_age_days / 30.44
                )  # Promedio de días por mes

                has_contact_info = bool(
                    row.email or row.phone or row.contact_person
                )
                ...
                client_info = {
                    ...
                    "total_estimated_hours": float(
                        row.total_estimated_hours or 0
                    ),
                    "client_age_months": round(client_age_months, 1),
                    "has_contact_info": has_contact_info,
                    "last_project_date": row.last_project_date.isoformat()
                ...
                if client_age_months < 1:  # Menos de 1 mes
                    ...
                    row.project_count >= avg_projects * 1.5
                    and row.total_estimated_hours > 100
                ):
            ...
            for segment_name, clients in segments.items():
                if clients:
                    ...
                    total_hours = sum(
                        c["total_estimated_hours"] for c in clients
                    )
                    avg_age = sum(
                        c["client_age_months"] for c in clients
                    ) / len(clients)
                    contact_info_rate = (
                        sum(1 for c in clients if c["has_contact_info"])
                        / len(clients)
                        * 100
                    )
                    ...
                    segment_stats[segment_name] = {
                        "count": len(clients),
                        "percentage": round(
                            len(clients) / len(clients_data) * 100, 2
                        ),
                        "total_projects": total_projects,
                        "total_estimated_hours": total_hours,
                        "avg_projects_per_client": round(
                            total_projects / len(clients), 2
                        ),
                        "avg_hours_per_client": round(
                            total_hours / len(clients), 2
                        ),
                        "avg_client_age_months": round(avg_age, 1),
                        "contact_info_completion_rate": round(
                            contact_info_rate, 2
                        ),
                        "clients": clients[
                            :5
                        ],  # Solo los primeros 5 para el reporte
                    }
                else:
                    segment_stats[segment_name] = {
                        "count": 0,
            ...
            self._logger.debug(
                f"Análisis de segmentación completado para {len(clients_data)} clientes"
            )
            return analysis

        except SQLAlchemyError as e:
            self._logger.error(
                f"Error de base de datos en análisis de segmentación: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado en análisis de segmentación: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="client_segmentation_analysis",
                parameters={},
                reason=str(e),
            )
        ...
        try:
            ...
            query = (
                select(
                    ...
                    func.coalesce(func.avg(Project.estimated_hours), 0).label(
            ...
            total_hours_list = [
                float(row.total_hours) for row in clients_with_projects
            ]
            project_counts_list = [
                row.project_count for row in clients_with_projects
            ]
            ...
            p75_hours = (
                sorted_hours[int(len(sorted_hours) * 0.75)]
                if sorted_hours
                else 0
            )
            p90_hours = (
                sorted_hours[int(len(sorted_hours) * 0.90)]
                if sorted_hours
                else 0
            )
            p75_projects = (
                sorted_projects[int(len(sorted_projects) * 0.75)]
                if sorted_projects
                else 0
            )
            ...
            for row in clients_with_projects:
                ...
                client_info = {
                    ...
                    "avg_hours_per_project": round(
                        float(row.avg_hours_per_project), 2
                    ),
                    "projects_per_year": round(projects_per_year, 2),
                    "client_age_days": client_age_days,
                    "first_project_date": row.first_project_date.isoformat()
                    if row.first_project_date
                    else None,
                    "last_project_date": row.last_project_date.isoformat()
                    if row.last_project_date
                    else None,
            ...
            for tier_name, clients in value_tiers.items():
                if clients:
                    ...
                    total_tier_projects = sum(
                        c["project_count"] for c in clients
                    )
                    avg_projects_per_year = sum(
                        c["projects_per_year"] for c in clients
                    ) / len(clients)

                    tier_stats[tier_name] = {
                        "count": len(clients),
                        "percentage": round(
                            len(clients) / total_clients * 100, 2
                        ),
                        "total_hours": total_tier_hours,
                        "total_projects": total_tier_projects,
                        "avg_hours_per_client": round(
                            total_tier_hours / len(clients), 2
                        ),
                        "avg_projects_per_client": round(
                            total_tier_projects / len(clients), 2
                        ),
                        "avg_projects_per_year": round(
                            avg_projects_per_year, 2
                        ),
                        "top_clients": sorted(
                            clients,
            ...
            analysis = {
                ...
                "benchmarks": {
                    ...
                    "avg_hours_per_client": round(
                        sum(total_hours_list) / len(total_hours_list), 2
                    ),
                    "avg_projects_per_client": round(
                        sum(project_counts_list) / len(project_counts_list), 2
                    ),
                },
                "analysis_timestamp": current_time.isoformat(),
            }
            ...
            self._logger.debug(
                f"Análisis de valor completado para {total_clients} clientes"
            )
            return analysis

        except SQLAlchemyError as e:
            self._logger.error(
                f"Error de base de datos en análisis de valor: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            raise create_client_statistics_error(
                statistic_type="client_value_analysis",
                parameters={},
                reason=str(e),
            ) from e
        ...
        try:
            ...
            for period_name, period_start in periods.items():
                ...
                for client in all_clients:
                    ...
                    if (
                        client.created_at
                        and client.created_at >= period_start.date()
                    ):
                    ...
                    # Cliente con actividad en este período
                        ...
                        and client.last_project_date >= period_start.date()
                    ):
                    ...
                    ):  # Solo considerar clientes que han tenido proyectos
                ...
                total_existing_clients = len(
                    [
                        c
                        for c in all_clients
                        if c.created_at and c.created_at < period_start.date()
                    ]
                )
                retention_rate = (
                    (len(active_in_period) / total_existing_clients * 100)
                    if total_existing_clients > 0
                    else 0
                )
            ...
            retention_trend = {
                "improving": retention_data["last_30_days"]["retention_rate"]
                > retention_data["last_90_days"]["retention_rate"],
                "stable": abs(
                    retention_data["last_30_days"]["retention_rate"]
                    - retention_data["last_90_days"]["retention_rate"]
                )
                <= 5,
                "declining": retention_data["last_30_days"]["retention_rate"]
                < retention_data["last_90_days"]["retention_rate"],
            }
            ...
            self._logger.debug(
                f"Análisis de retención completado para {len(all_clients)} clientes"
            )
            return analysis

        except SQLAlchemyError as e:
            self._logger.error(
                f"Error de base de datos en análisis de retención: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado en análisis de retención: {e}"
            )
            raise create_client_statistics_error(
    ...
    async def get_comprehensive_dashboard_metrics(self) -> dict[str, Any]:
        ...
        try:
            self._logger.debug(
                "Iniciando generación de métricas comprehensivas para dashboard"
            )
            ...
            creation_trends_task = self.get_client_creation_trends(
                group_by="month", limit=6
            )
            ...
            monthly_trends = (
                creation_trends[-2:]
                if len(creation_trends) >= 2
                else creation_trends
            )
            growth_rate = 0
            ...
            sum(
                tier["count"]
                for tier in value_analysis["value_tiers"].values()
            )
            premium_percentage = (
                value_analysis["value_tiers"]["premium"]["percentage"]
                if "premium" in value_analysis["value_tiers"]
                else 0
            )

            # KPIs de retención
            retention_30_days = (
                retention_analysis["retention_by_period"]["last_30_days"][
                    "retention_rate"
                ]
                if "last_30_days" in retention_analysis["retention_by_period"]
                else 0
            )
            ...
            dashboard_metrics = {
                "executive_summary": {
                    ...
                    "contact_completion_rate": contact_stats[
                        "contact_info_percentage"
                    ],
                    "clients_with_active_projects": activity_summary[
                        "clients_with_active_projects"
                    ],
                },
                "detailed_analytics": {
                ...
                "alerts_and_insights": {
                    ...
                    "contact_info_incomplete_alert": contact_stats[
                        "contact_info_percentage"
                    ]
                    < 80,
                    "dormant_clients_count": segmentation["segments"][
                        "dormant"
                    ]["count"],
                    "high_value_clients_count": (
                        segmentation["segments"]["high_value"]["count"]
                        + segmentation["segments"]["premium"]["count"]
                        if "premium" in segmentation["segments"]
                        else 0
                    ),
                },
                "generated_at": current_time.isoformat(),
                "data_freshness": {
                    ...
                    "next_update_recommended": current_time.add(
                        days=7
                    ).isoformat(),
                },
            }
            ...
            self._logger.debug(
                "Métricas comprehensivas de dashboard generadas exitosamente"
            )
            return dashboard_metrics

        except SQLAlchemyError as e:
            self._logger.error(
                f"Error de base de datos generando métricas de dashboard: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado generando métricas de dashboard: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="comprehensive_dashboard_metrics",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_performance_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            duration_stats = (
                await self.get_client_relationship_duration_stats()
            )
            ...
            recent_clients_result = await self.session.execute(
            ...
            previous_clients_result = await self.session.execute(
                previous_clients_query
            )
            ...
            self._logger.error(
                f"Error de base de datos generando métricas de rendimiento: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado generando métricas de rendimiento: {e}"
            )
            raise create_client_statistics_error(
                statistic_type="performance_metrics",
                parameters={},
                reason=str(e),
            )

    # ============================================================================
    # MÉTODOS ABSTRACTOS REQUERIDOS POR IClientStatistics
    # ============================================================================
    ...
    async def get_active_client_count(self) -> int:
        ...
        """
        try:
            query = select(func.count(Client.id)).where(Client.is_active == True)
            result = await self.session.execute(query)
            return result.scalar() or 0
        except SQLAlchemyError as e:
            self._logger.error(f"Error obteniendo conteo de clientes activos: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado obteniendo conteo de clientes activos: {e}")
            raise create_client_statistics_error(
                statistic_type="active_client_count",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_clients_by_creation_date(
        ...
        try:
            query = (
                select(
                    func.date(Client.created_at).label("creation_date"),
                ...
                .order_by(func.date(Client.created_at))
            )
            ...
            self._logger.error(f"Error obteniendo clientes por fecha de creación: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado obteniendo clientes por fecha: {e}")
            raise create_client_statistics_error(
                statistic_type="clients_by_creation_date",
    ...
    async def get_top_clients_by_projects(
        ...
        """
        # Reutilizar método existente
        return await self.get_clients_by_project_count(limit or 10)

    async def calculate_client_metrics(self) -> dict[str, Any]:
        """Calcula métricas generales de clientes.
        ...
        try:
            ...
            active_clients = await self.get_active_client_count()
            inactive_clients = total_clients - active_clients
            ...
            inactive_percentage = (
                (inactive_clients / total_clients * 100) if total_clients > 0 else 0
            )

            # Obtener estadísticas adicionales
            status_distribution = await self.get_client_distribution_by_status()
            activity_summary = await self.get_client_activity_summary()
            ...
            return {
                ...
                "activity_summary": activity_summary,
                "calculation_timestamp": get_current_time().isoformat(),
            }
        except Exception as e:
            self._logger.error(f"Error calculando métricas de clientes: {e}")
            raise create_client_statistics_error(
                statistic_type="client_metrics",
                parameters={},
                reason=str(e),
            )

    async def get_monthly_client_growth(self) -> list[dict[str, Any]]:
        """Obtiene el crecimiento mensual de clientes.

        Returns:
            Lista con datos de crecimiento mensual.
        """
        try:
            # Obtener datos de los últimos 12 meses
            current_time = get_current_time()
            start_date = current_time.subtract(months=12)
            ...
            query = (
                ...
                .where(Client.created_at >= start_date)
                .group_by(func.date_trunc("month", Client.created_at))
                .order_by(func.date_trunc("month", Client.created_at))
            )
            ...
            for row in result:
                cumulative_total += row.new_clients
                monthly_data.append(
                    {
                        ...
                        "growth_rate": (
                            (
                                (row.new_clients / monthly_data[-1]["new_clients"] - 1)
                                * 100
                            )
                            if len(monthly_data) > 0
                            and monthly_data[-1]["new_clients"] > 0
                            else 0
                        ),
                    }
                )
            ...
            self._logger.error(f"Error obteniendo crecimiento mensual: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado obteniendo crecimiento mensual: {e}")
            raise create_client_statistics_error(
                statistic_type="monthly_growth",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_total_clients_count(self) -> int:
        ...
        try:
            ...
            self._logger.error(f"Error inesperado obteniendo conteo de clientes: {e}")
            raise create_client_statistics_error(
                statistic_type="total_client_count",
                parameters={},
                reason=str(e),
            )
    ...
    async def get_active_clients_count(self) -> int:
        ...
        try:
            ...
            self._logger.error(f"Error inesperado obteniendo conteo de clientes activos: {e}")
            raise create_client_statistics_error(
                statistic_type="active_clients_count",
                parameters={},
                reason=str(e),
            )

    async def get_inactive_clients_count(self) -> int:
        ...
        """
        try:
            query = select(func.count(Client.id)).where(Client.is_active == False)
            result = await self.session.execute(query)
            return result.scalar() or 0
        except SQLAlchemyError as e:
            self._logger.error(f"Error obteniendo conteo de clientes inactivos: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado obteniendo conteo de clientes inactivos: {e}")
            raise create_client_statistics_error(
                statistic_type="inactive_client_count",
                parameters={},
                reason=str(e),
            )

    async def get_clients_created_this_month(self) -> int:
        """Obtiene el número de clientes creados este mes.
        
        Returns:
            Número de clientes creados en el mes actual.
        """
        try:
            current_time = get_current_time()
            start_of_month = current_time.start_of('month')
            ...
            self._logger.error(f"Error obteniendo clientes creados este mes: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado obteniendo clientes del mes: {e}")
            raise create_client_statistics_error(
                message=f"Error obteniendo clientes creados este mes: {e}",
                original_error=e,
            )

    async def get_clients_created_this_year(self) -> int:
        """Obtiene el número de clientes creados este año.
        
        Returns:
            Número de clientes creados en el año actual.
        """
        try:
            current_time = get_current_time()
            start_of_year = current_time.start_of('year')
            end_of_year = current_time.end_of('year')
            
            query = select(func.count(Client.id)).where(
                and_(
                    Client.created_at >= start_of_year,
                    Client.created_at <= end_of_year
                )
            )
            result = await self.session.execute(query)
            return result.scalar() or 0
        except SQLAlchemyError as e:
            self._logger.error(f"Error obteniendo clientes creados este año: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado obteniendo clientes del año: {e}")
            raise create_client_statistics_error(
                statistic_type="clients_created_this_year",
                parameters={},
                reason=str(e),
            )

    async def get_average_clients_per_month(self) -> float:
        """Calcula el promedio de clientes por mes.
        ...
        try:
            ...
            months_active = await self._get_months_since_first_client()
            if months_active == 0:
                return float(total_clients)  # Primer mes
                
            return total_clients / months_active
        except Exception as e:
            self._logger.error(f"Error calculando promedio de clientes por mes: {e}")
            raise create_client_statistics_error(
                statistic_type="average_clients_per_month",
                parameters={},
                reason=str(e),
            )

    async def _get_months_since_first_client(self) -> int:
        """Obtiene el número de meses desde el primer cliente.
        
        Returns:
            Número de meses desde el primer cliente.
        """
        try:
            query = select(func.min(Client.created_at))
            result = await self.session.execute(query)
            first_client_date = result.scalar()
            
            # Manejar caso de corrutina (para tests con mocks)
            if hasattr(first_client_date, '__await__'):
                first_client_date = await first_client_date
            
            if not first_client_date:
                return 1  # Retornar 1 mes como mínimo cuando no hay clientes
                
            current_time = get_current_time()
            
            # Convertir ambas fechas a pendulum de manera robusta
            import pendulum
            
            # Asegurar que current_time sea pendulum
            if isinstance(current_time, pendulum.DateTime):
                current_pendulum = current_time
            else:
                current_pendulum = pendulum.instance(current_time)
            
            # Asegurar que first_client_date sea pendulum
            if isinstance(first_client_date, pendulum.DateTime):
                first_client_pendulum = first_client_date
            else:
                # Convertir datetime a pendulum
                first_client_pendulum = pendulum.instance(first_client_date)
                
            # Calcular diferencia en meses
            months_diff = current_pendulum.diff(first_client_pendulum).in_months()
            return max(1, months_diff)  # Mínimo 1 mes
        except SQLAlchemyError as e:
            self._logger.error(f"Error obteniendo meses desde primer cliente: {e}")
            raise convert_sqlalchemy_error(e)
        except Exception as e:
            self._logger.error(f"Error inesperado calculando meses: {e}")
            raise create_client_statistics_error(
                statistic_type="months_since_first_client",
                parameters={},
                reason=str(e),
            )

    async def calculate_growth_rate(self, current_period: int, previous_period: int) -> float:
        """Calcula la tasa de crecimiento entre dos períodos.
        
        Args:
            current_period: Valor del período actual.
            previous_period: Valor del período anterior.
        ...
        if current_period == 0 and previous_period == 0:
            return 0.0
            
        growth_rate = ((current_period - previous_period) / previous_period) * 100
        return round(growth_rate, 2)

    async def get_monthly_client_creation_stats(self, year: int | None = None) -> dict[int, int]:
        """Obtiene estadísticas de creación de clientes por mes.
        
        Args:
            year: Año para filtrar (opcional, por defecto año actual).
            
        Returns:
            Diccionario con mes como clave y número de clientes como valor.
        """
        try:
            if year is None:
                year = get_current_time().year
                
            # Crear query para obtener datos mensuales
            query = (
                select(
                    func.extract('month', Client.created_at).label('month'),
                    func.count(Client.id).label('count')
                )
                .where(func.extract('year', Client.created_at) == year)
                .group_by(func.extract('month', Client.created_at))
            )
            
            result = await self.session.execute(query)
            monthly_data = {row.month: row.count for row in result.fetchall()}
            
            # Completar todos los meses (1-12) con 0 si no hay datos
            complete_data = {}
            for month in range(1, 13):
                complete_data[month] = monthly_data.get(month, 0)
            ...
            raise create_client_statistics_error(
                statistic_type="monthly_client_creation_stats",
                parameters={"year": year},
                reason=str(e),
            )

    async def generate_comprehensive_stats(self) -> dict[str, Any]:
        """Genera estadísticas comprehensivas de clientes.
        ...
        try:
            ...
            inactive_clients = await self.get_inactive_clients_count()
            clients_this_month = await self.get_clients_created_this_month()
            clients_this_year = await self.get_clients_created_this_year()
            average_clients_per_month = await self.get_average_clients_per_month()
            
            # Calcular tasa de crecimiento (mes actual vs mes anterior)
            current_time = get_current_time()
            previous_month_start = current_time.subtract(months=1).start_of('month')
            previous_month_end = current_time.subtract(months=1).end_of('month')
            
            query_prev_month = select(func.count(Client.id)).where(
                and_(
                    Client.created_at >= previous_month_start,
                    Client.created_at <= previous_month_end
                )
            )
            result_prev = await self.session.execute(query_prev_month)
            clients_previous_month = result_prev.scalar() or 0
            
            growth_rate = await self.calculate_growth_rate(
                clients_this_month, clients_previous_month
            )
            ...
            return {
                ...
                "average_clients_per_month": average_clients_per_month,
                "growth_rate": growth_rate,
                "calculation_timestamp": get_current_time().isoformat(),
            }
        except Exception as e:
            self._logger.error(f"Error generando estadísticas comprehensivas: {e}")
            raise create_client_statistics_error(
                statistic_type="comprehensive_stats",
                parameters={},
                reason=str(e),
            )

    async def get_top_clients_by_activity(self, limit: int = 10) -> list[Client]:
        ...
        Returns:
            Lista de clientes ordenados por actividad.
        """
        try:
            # Usar el método existente que ya ordena por número de proyectos
            clients_data = await self.get_clients_by_project_count(limit)
            
            # Extraer solo los IDs de cliente y obtener los objetos Client
            client_ids = [client_data["id"] for client_data in clients_data]
            ...
            self._logger.error(f"Error obteniendo top clientes por actividad: {e}")
            raise create_client_statistics_error(
                statistic_type="top_clients_by_activity",
                parameters={"limit": limit},
                reason=str(e),
            )

    def validate_date_range(self, start_date: Any, end_date: Any) -> None:
        ...
        # Convertir a pendulum si es necesario
        if hasattr(start_date, 'year') and not hasattr(start_date, 'add'):
            start_date = instance(start_date)
        elif isinstance(start_date, str):
            start_date = parse(start_date)
            
        if hasattr(end_date, 'year') and not hasattr(end_date, 'add'):
            end_date = instance(end_date)
        elif isinstance(end_date, str):
            end_date = parse(end_date)
            
        # Convertir a Date para comparaciones consistentes
        ...
        current_time = get_current_time()
        current_date_only = current_time.date() if hasattr(current_time, 'date') else current_time
        
        if start_date_only > current_date_only:
            raise ValueError(
                f"La fecha de inicio ({start_date}) no puede ser futura"
            )
            
        if end_date_only > current_date_only:
            raise ValueError(
                f"La fecha de fin ({end_date}) no puede ser futura"
            )
    
    async def export_stats_to_dict(self) -> dict[str, Any]:
        """Exporta todas las estadísticas principales a un diccionario.
        
        Returns:
            Diccionario con todas las estadísticas principales.
        """
        try:
            ...
            current_year_clients = await self.get_clients_created_this_year()
            monthly_stats = await self.get_monthly_client_creation_stats()
            
            return {
                "basic_counts": basic_stats,
                "activity_summary": activity_summary,
                "totals": {
                    "total_clients": total_clients,
                    "active_clients": active_clients,
                    "current_year_clients": current_year_clients
                },
                "monthly_creation_stats": monthly_stats,
                "export_timestamp": get_current_time().isoformat()
            }
        except Exception as e:
            self._logger.error(f"Error exportando estadísticas: {e}")
            raise create_client_statistics_error(
                statistic_type="export_stats",
                parameters={},
                reason=str(e),
            )
