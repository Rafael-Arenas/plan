...
class ClientDateOperations:
    ...
    def __init__(
        self, session: AsyncSession, query_builder: ClientQueryBuilder
    ):
    ...
    async def get_clients_created_current_week(self) -> list[Client]:
        ...
        try:
            ...
            self._logger.error(
                f"Error obteniendo clientes de semana actual: {e}"
            )
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes de semana actual: {e}"
            )
    ...
    async def get_clients_created_current_month(self) -> list[Client]:
        ...
        try:
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes de mes actual: {e}"
            )
            raise RepositoryError(
                message=(
                    f"Error inesperado obteniendo clientes de mes actual: {e}"
    ...
    async def get_clients_created_business_days_only(self) -> list[Client]:
        ...
        Filtra clientes creados únicamente en días laborables
        (lunes a viernes),
        ...
        try:
            ...
            query = (
                ...
                .where(
                    and_(
                        ...
                        func.strftime("%w", Client.created_at).between(
                            "1", "5"
                        ),
            ...
            for client in clients:
                if client.created_at:
                    ...
                    if (
                        client_date.weekday() < 5
                    ):  # 0-4 son días laborables en Pendulum
            ...
            self._logger.error(
                f"Error obteniendo clientes de días laborables: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes de días laborables: {e}"
            )
            raise RepositoryError(
    ...
    async def get_clients_by_date_range(
        ...
        try:
            ...
            # Re-lanzar ValueError para validaciones sin modificar
            raise
        except SQLAlchemyError as e:
            self._logger.error(
                f"Error obteniendo clientes por rango de fechas: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes por rango de fechas: {e}"
            )
            raise RepositoryError(
    ...
    async def get_client_creation_trends(
        ...
        try:
            ...
            query = (
                select(
                    func.strftime(format_str, Client.created_at).label(
                        "period"
                    ),
                    func.count(Client.id).label("count"),
                )
                .where(Client.created_at.isnot(None))
                .group_by(func.strftime(format_str, Client.created_at))
                .order_by(func.strftime(format_str, Client.created_at).desc())
                .limit(limit)
            )
            ...
            for row in trends_data:
                ...
                try:
                    if period == "day":
                        ...
                        trend_item["formatted_date"] = parsed_date.format(
                            "DD/MM/YYYY"
                        )
                    elif period == "month":
    ...
    async def get_clients_by_age_range(
        ...
        try:
            ...
            self._logger.error(
                f"Error obteniendo clientes por rango de edad: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado obteniendo clientes por rango de edad: {e}"
            )
            raise RepositoryError(
    ...
    async def calculate_client_age(self, client_id: int) -> int | None:
        ...
        try:
            ...
            self._logger.error(
                f"Error calculando edad del cliente {client_id}: {e}"
            )
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(
                f"Error inesperado calculando edad del cliente {client_id}: {e}"
            )
            raise RepositoryError(
