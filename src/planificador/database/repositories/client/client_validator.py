...
class ClientValidator:
    ...
    def validate_client_data(self, client_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'name' in validated_data:
                validated_data['name'] = self._validate_name(validated_data['name'])
            
            # Validar y normalizar código
            if 'code' in validated_data:
                validated_data['code'] = self._validate_code(validated_data['code'])
            
            # Validar descripción
            if 'description' in validated_data and validated_data['description']:
                validated_data['description'] = self._validate_description(validated_data['description'])
            
            # Validar información de contacto
            if 'contact_info' in validated_data and validated_data['contact_info']:
                validated_data['contact_info'] = self._validate_contact_info(validated_data['contact_info'])
            
            # Validar email si se proporciona
            if 'email' in validated_data and validated_data['email']:
                validated_data['email'] = self._validate_email(validated_data['email'])
            
            # Validar teléfono si se proporciona
            if 'phone' in validated_data and validated_data['phone']:
                validated_data['phone'] = self._validate_phone(validated_data['phone'])
            
            # Validar estado activo
            if 'is_active' in validated_data:
                validated_data['is_active'] = self._validate_is_active(validated_data['is_active'])
            
            # Aplicar reglas de negocio
            self._apply_business_rules(validated_data)
            
            self._logger.debug(f"Datos de cliente validados exitosamente: {validated_data.get('name', 'N/A')}")
            return validated_data
            
        except ValidationError:
            raise
        except Exception as e:
            self._logger.error(f"Error inesperado validando datos de cliente: {e}")
            raise ValidationError(f"Error de validación: {str(e)}")
    
    def validate_bulk_data(self, clients_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        if len(clients_data) > 100:  # Límite para operaciones en lote
            raise ValidationError("No se pueden procesar más de 100 clientes a la vez")
        ...
        for i, client_data in enumerate(clients_data):
            try:
                ...
                errors.append(f"Cliente {i + 1}: {str(e)}")
        
        if errors:
            raise ValidationError(f"Errores de validación en lote: {'; '.join(errors)}")
        
        # Validar nombres únicos dentro del lote
        names = [client.get('name', '').lower() for client in validated_clients]
        if len(names) != len(set(names)):
            raise ValidationError("Hay nombres duplicados en el lote de clientes")
        
        # Validar códigos únicos dentro del lote
        codes = [client.get('code', '').upper() for client in validated_clients if client.get('code')]
        if len(codes) != len(set(codes)):
            raise ValidationError("Hay códigos duplicados en el lote de clientes")
        
        self._logger.debug(f"Validados {len(validated_clients)} clientes en lote")
    ...
    def validate_update_data(self, update_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(update_data, dict):
            raise ValidationError("Los datos de actualización deben ser un diccionario")
        ...
        for field, value in update_data.items():
            if field == 'name' and value is not None:
                ...
                validated_data['description'] = self._validate_description(value)
            elif field == 'contact_info' and value is not None:
                validated_data['contact_info'] = self._validate_contact_info(value)
        ...
        self._logger.debug(f"Datos de actualización validados: {list(validated_data.keys())}")
        return validated_data
    ...
    def validate_creation_business_day(self, validate_business_day: bool = False) -> None:
        """
    ...
    async def validate_name_exists(self, name: str, exclude_id: Optional[int] = None) -> bool:
        """
        ...
        """
        if not self.session:
            raise ValidationError("Sesión de base de datos requerida para validación")
        ...
        try:
            ...
            self._logger.error(f"Error de BD validando existencia de nombre de cliente: {e}")
            ...
            self._logger.error(f"Error inesperado validando existencia de nombre de cliente: {e}")
            raise create_client_validation_repository_error(
                message=f"Error inesperado validando nombre: {e}",
                details={"name": name, "exclude_id": exclude_id, "original_error": str(e)}
            )
    
    async def validate_code_exists(self, code: str, exclude_id: Optional[int] = None) -> bool:
        """
        ...
        try:
            ...
            self._logger.error(f"Error de BD validando existencia de código de cliente: {e}")
            raise convert_sqlalchemy_error(
                error=e,
            ...
            self._logger.error(f"Error inesperado validando existencia de código de cliente: {e}")
            raise create_client_validation_repository_error(
                message=f"Error inesperado validando código: {e}",
                details={"code": code, "exclude_id": exclude_id, "original_error": str(e)}
            )
    
    async def validate_client_update_data(self, update_data: Dict[str, Any], client_id: int) -> Dict[str, Any]:
        """
        ...
        if 'name' in validated_data and self.session:
            name_exists = await self.validate_name_exists(validated_data['name'], client_id)
            if name_exists:
                raise ValidationError(f"Ya existe un cliente con el nombre '{validated_data['name']}'")
        
        # Validar unicidad de código si se está actualizando
        if 'code' in validated_data and self.session:
            code_exists = await self.validate_code_exists(validated_data['code'], client_id)
            if code_exists:
                raise ValidationError(f"Ya existe un cliente con el código '{validated_data['code']}'")
    ...
    def _validate_contact_info(self, contact_info: str) -> str:
        ...
        if not isinstance(contact_info, str):
            raise ValidationError("La información de contacto debe ser una cadena de texto")
