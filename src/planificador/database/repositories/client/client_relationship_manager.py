...
from sqlalchemy import and_, or_, select, func, update, delete, case
...
class ClientRelationshipManager:
    ...
    async def get_client_projects(
        ...
        try:
            ...
            self._logger.error(
                f"Error inesperado obteniendo proyectos del cliente {client_id}: {e}"
            )
            raise RepositoryError(
                message=f"Error inesperado obteniendo proyectos del cliente: {e}",
                operation="get_client_projects",
                entity_type="Client",
                entity_id=client_id,
                original_error=e
    ...
    async def assign_project_to_client(
        ...
        try:
            ...
            if not project:
                raise ValidationError(f"Proyecto con ID {project_id} no encontrado")
            ...
            self._logger.info(
                f"Proyecto {project_id} asignado exitosamente al cliente {client_id}"
            )
            return project
            
        except (ValidationError, ClientNotFoundError):
            raise
        except SQLAlchemyError as e:
            self._logger.error(f"Error de base de datos asignando proyecto {project_id} al cliente {client_id}: {e}")
    ...
    async def transfer_projects_to_client(
        ...
        Raises:
            ...
            ClientConflictError: Si hay problemas con la transferencia
        """
        try:
            ...
            if project_ids:
                ...
                query = select(Project).where(Project.client_id == source_client_id)
            ...
            if validate_transfer:
                for project in projects_to_transfer:
                    await self._validate_project_assignment(project, target_client_id)
    ...
    async def get_client_project_summary(
        ...
        try:
            ...
            projects_query = select(
                ...
                func.sum(case((Project.status == 'planned', 1), else_=0)).label('planned'),
                func.sum(case((Project.status == 'in_progress', 1), else_=0)).label('in_progress'),
                ...
                func.sum(case((Project.is_active == True, 1), else_=0)).label('active'),
            ...
            recent_projects_query = select(Project).where(
                Project.client_id == client_id
            ).order_by(
            ...
            summary = {
                ...
                'project_statistics': {
                    ...
                    'inactive_projects': (stats.total or 0) - (stats.active or 0)
                },
                'timeline': {
                    'first_project_date': (
                        stats.first_project_date.isoformat() 
                        if stats.first_project_date else None
                    ),
                    'last_project_date': (
                        stats.last_project_date.isoformat() 
                        if stats.last_project_date else None
                    )
                },
                'recent_projects': [
                    {
            ...
            self._logger.debug(f"Resumen de proyectos generado para cliente {client_id}")
            return summary
            
        except ClientNotFoundError:
            raise
        except SQLAlchemyError as e:
            self._logger.error(f"Error de base de datos generando resumen de proyectos para cliente {client_id}: {e}")
            raise convert_sqlalchemy_error(
                error=e,
    ...
    async def validate_client_project_integrity(
        ...
        try:
            ...
            orphaned_result = await self.session.execute(orphaned_projects_query)
            ...
            if not client.is_active:
                ...
                active_result = await self.session.execute(active_projects_query)
            ...
            invalid_dates_result = await self.session.execute(invalid_dates_query)
    ...
    async def cleanup_client_relationships(
        ...
        Raises:
            ClientNotFoundError: Si el cliente no existe
            ClientValidationError: Si hay problemas con la limpieza
        """
        try:
            # Verificar que el cliente existe
            await self._validate_client_exists(client_id)
            ...
            if not projects:
                cleanup_result['actions_taken'].append('No hay proyectos para limpiar')
            ...
            await self.session.rollback()
            self._logger.error(f"Error de base de datos limpiando relaciones del cliente {client_id}: {e}")
            raise convert_sqlalchemy_error(
                error=e,
    ...
    async def _validate_project_assignment(
        ...
        Raises:
            ClientConflictError: Si la asignación no es válida
        """
